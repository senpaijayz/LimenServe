...
import { getAdminNotifications } from '../../../services/notificationsApi';
    ...
    sections: [
      ...
      },
      {
        ...
        content: {
          ...
          title: 'Popular Mitsubishi parts with clear fitment guidance',
          primaryActionLabel: 'Add to Quote',
          secondaryActionLabel: 'Quick View',
          items: [
            { title: 'Mitsubishi Oil Filter', description: 'Engine-oil filtration for compatible Pajero and Montero diesel applications. Confirm engine and chassis fitment before ordering.', eyebrow: 'Pajero / Montero', badge: 'Genuine reference', partNo: 'ME013307', price: 'PHP 3,148', href: '/catalog?q=ME013307', imageUrl: 'https://motorpartsjunction.in/cdn/shop/files/WhatsAppImage2024-03-15at17.01.56_09285caf.jpg?v=1710643991', imageAlt: 'Mitsubishi ME013307 oil filter' },
            { title: 'Front Brake Pad Set', description: 'Front-axle pad set for compatible Pajero and Montero applications. Replace as a complete axle set and verify fitment by chassis number.', eyebrow: 'Pajero / Montero', badge: 'Safety essential', partNo: 'MZ690572', price: 'PHP 4,478', href: '/catalog?q=MZ690572', imageUrl: 'https://partschai.com/public/uploads/all/2YzhtqHOSDQQYK6JeKOoDqv58JMfDOS3BfngvmDT.jpg', imageAlt: 'Mitsubishi MZ690572 front brake pad set' },
            { title: 'Engine Air Cleaner Element', description: 'Pleated intake filter for compatible Xpander and Xforce applications. Replace when loaded with dust or at the scheduled service interval.', eyebrow: 'Xpander / Xforce', badge: 'Service item', partNo: '1500A687', price: 'PHP 1,505', href: '/catalog?q=1500A687', imageUrl: 'https://cdn.phutungmitsubishi.vn/media/xpander%201/l%E1%BB%8Dc%20gió%20%C4%91%E1%BB%99ng%20c%C6%A1%20xe%20mitsubishi%20xpander%2C%20xpander%20cross%20v%C3%A0%20xforce%20h%C3%A0ng%20koto%20%281%29.jpg', imageAlt: 'Mitsubishi 1500A687 engine air cleaner element' },
            { title: 'Cabin Air Filter', description: 'Pleated HVAC filter that helps reduce dust and pollen in the cabin. Check the airflow arrow and vehicle fitment before installation.', eyebrow: 'Xpander / Mirage', badge: 'Service item', partNo: 'MZ691067', price: 'PHP 493', href: '/catalog?q=MZ691067', imageUrl: 'https://i.ebayimg.com/images/g/VR4AAOSwuLFkwTcn/s-l500.png', imageAlt: 'Mitsubishi MZ691067 cabin air filter in genuine packaging' },
          ],
        },
      },
      {
  ...
  return (
    <div className="space-y-6 p-5 lg:p-6">
      ...
      <section className="rounded-3xl border border-primary-200 bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-xl font-display font-semibold text-primary-950">Recent activity</h3>
            <p className="mt-1 text-sm text-primary-500">Operational events generated by existing admin workflows.</p>
          </div>
          {notifications && <span className="rounded-full bg-primary-100 px-3 py-1 text-xs font-bold text-primary-600">{notifications.unreadCount} unread</span>}
        </div>

        {notifications === null ? (
          <div className="mt-5 rounded-2xl border border-dashed border-primary-300 bg-primary-50 p-5 text-sm text-primary-600">Activity is temporarily unavailable.</div>
        ) : notifications.notifications.length === 0 ? (
          <div className="mt-5 rounded-2xl border border-dashed border-primary-300 bg-primary-50 p-5 text-sm text-primary-600">No recent activity has been recorded yet.</div>
        ) : (
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {notifications.notifications.slice(0, 8).map((notification) => (
              <Link key={notification.id} to={notification.targetPath || '/dashboard'} className="rounded-2xl border border-primary-200 bg-primary-50/60 p-4 transition hover:border-accent-primary hover:bg-white">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-semibold text-primary-950">{notification.title || 'Admin activity'}</p>
                  <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${notification.read ? 'bg-primary-200' : 'bg-accent-danger'}`} title={notification.read ? 'Read' : 'Unread'} />
                </div>
                <p className="mt-1 text-sm leading-relaxed text-primary-600">{notification.message || 'Open the linked workflow for details.'}</p>
                {notification.createdAt && <time className="mt-3 block text-xs text-primary-400">{new Date(notification.createdAt).toLocaleString('en-PH')}</time>}
              </Link>
            ))}
          </div>
        )}
