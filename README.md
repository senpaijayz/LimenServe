...
New quotations receive a non-sequential numeric `EST-########` identifier in the database. Existing identifiers are preserved. Staff saved-quotation cards now show the server `created_at` timestamp, omit the internal revision-count aggregate, and provide a larger red hover state for draft deletion. The forward migrations and database invariants are `supabase/migrations/20260820170000_random_estimate_numbers_and_created_date.sql`, `supabase/migrations/20260820171000_harden_estimate_card_functions.sql`, and `supabase/tests/20260820_estimate_card_invariants.sql`.
